      ...
      <div class="col-8 ml-col-2">
          ...
          <form class="" action="{{ route('update.rental',$rental->id) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
              ...
              <input type="number"  min="1" max="10" class="form-control {{$errors->has('rooms') ? "border-danger" : "" }}" name = "rooms" value="{{$rental->rooms}}">
                {!! $errors->first('rooms', '<small class="form-text text-danger">:message</small>') !!}
            </div>
            <div class="form-group">
              <label for="bedrooms">Beds</label>
              <input type="number"  min="1" max="10" class="form-control {{$errors->has('bedrooms') ? "border-danger" : "" }}" name = "beds" value="{{$rental->beds}}">
                {!! $errors->first('beds', '<small class="form-text text-danger">:message</small>') !!}
            </div>
            <div class="form-group">
              <label for="bathrooms">Bathrooms</label>
              <input type="number"  min="1" max="10" class="form-control {{$errors->has('bathrooms') ? "border-danger" : "" }}" name = "bathrooms" value={{$rental->bathrooms}}>
                {!! $errors->first('bathrooms', '<small class="form-text text-danger">:message</small>') !!}
            </div>
            <div class="form-group">
              <label for="square_meters">Surface (m<sup>2</sup>)</label>
              <input type="number"  step="0.01" class="form-control {{$errors->has('square_meters') ? "border-danger" : "" }}" name = "square_meters" value="{{$rental->square_meters}}">
